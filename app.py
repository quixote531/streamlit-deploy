...
# Step1. 결측치, 이상치 확인 및 처리 - 데이터셋 불러와서 우선 실시
...
# BMI 이상치 제거
# df = df[(조건식1) & (조건식2)]
df = df[(df['BMI'] >= lower_bound) & (df['BMI'] <= upper_bound)] #경계범위 내 데이터값만 불러옴
# st.dataframe(df)
...
df["Label"] = le_label.fit_transform(df["Label"]) #'Label'범주의 문자데이터를 숫자로 변환
...
# 랜덤 포레스트는 결정 트리(Decision Tree)를 여러개 조합하여 예측 정확도를 높이는 모델입니다.
...
# precision(정밀도): 모델이 '비만'이라고 예측한 것 중 실제로 맞는 비율 => '과체중'을 예측했을 때 실제로 과체중인 비율
# Recall(재현율) : 실제 '비만' 중에서 모델이 맞춘 비율 => '비만'인 사람을 놓치지 않고 얼마나 예측했는가?
# F1-score : Precision과 Recall의 조화평균 => 모델의 전체적인 성능을 나타냄
# Support 각 클래스(Label)에 속하는 샘플의 갯수 => 데이터셋 내에 존재하는 해당 클래스 갯수
...
def eda():
    ...
    with chart_tabs[1]:
        st.subheader("성별 및 비만 등급별 BMI 박스플롯")

        # 숫자형 Gender → 문자형 복원
        if df['Gender'].dtype in [int, float]:
            df['Gender'] = df['Gender'].map({0: 'Female', 1: 'Male'})

        # Label 컬럼이 숫자형이면 다시 문자형 라벨로 매핑
        label_map = {
            0: "Underweight",
            1: "Normal Weight",
            2: "Overweight",
